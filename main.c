...
/***************** Liste Olusturma Kismi *********************/	//buketecrinozturk
	struct node* listeOlustur(int data)
	{
	    ...
	    temp->next = NULL; // temp dügüm ilk ve tek dügüm oldu?u için next hücresine NULL atyyoruz.
	
	    head = last = temp; // geçici dü?dügüm ilk ve tek dügüm oldu?u için ayni zamanda son dügümde olmus oluyor
	    return temp;
	}
	...
	struct node* elemanArama(int data, struct node **prev)
	{
	    ...
	    if(true == found)
	    {
	        if(prev) // bu kysmy silme i?lemi için kullanyyoruz
	        *prev = tmp;
	        return list;
	    }
	    else
	...
	/***************** Listedeki Verileri Listeleme Kysmy *********************/ //buketecrinozturk	
